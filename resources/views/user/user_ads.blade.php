        ...
        <div class="row">
            <div class="col-sm-9 page-content">
                <div class="category-list">
                    ...
                    <div class="listing-filter">
                        <div class="pull-left col-xs-6">
                            <div class="breadcrumb-list"><a href="javascript:void(0)" class="current"> <span>Tüm kullanıcı ilanları</span></a>
                            </div>
                        </div>
                        ...
                        @foreach($search_fields as $item)
                            <div class="col-md-3">
                                ...
                                <select name="custom_search[{{ strtolower( str_replace(' ', '_', $item->name) ) }}]" form="search_form" onchange="$('#search_form').submit()" class="form-control">
                                    <option value="">Tüm {{ucfirst($item->name)}} </option>
                                        ...
                                        <div class="item-list make-list" style="">
                                            ...
                                            <div class="job-actions pull-right">
                                                <ul class="list-unstyled list-inline">
                                                        ...
                                                        <li>
                                                            <a class="save-add"  data-action="{{ (count($v->save_add) > 0)? 'del': 'ins' }}" data-id="{{$v->id}}" href="javascript:void(0)" title="" >
                                                                @if(count($v->save_add) > 0)
                                                                    ...
                                                                    <span class="text">Kaydedildi Ekle</span>
                                                                @else
                                                                    <span class="fa fa-star-o"></span>
                                                                    <span class="text">İlanı kaydet</span>
                                                                @endif
                                                    ...
                                                    <li class="saved-job">
                                                        @if($v->user->is_login)
                                                            <a class=""> <i class="fa fa-circle text-success"></i> <span>Çevrimiçi</span> </a>
                                                        @else
                                                            <a class=""> <i class="fa fa-circle text-danger"></i> <span>Çevrimdışı</span> </a>
                                                        @endif
                                                    </li>
                                                </ul>
                                            </div>
                ...
                <div class="post-promo text-center">
                    <h2> Satmak için bir şey alır mısın? </h2>
                    <h5>Ürünlerinizi çevrimiçi olarak ÜCRETSİZ olarak satın. Düşündüğünden daha kolay!</h5>
                    <a href="{{route('ads.create')}}" class="btn btn-lg btn-border btn-post btn-danger">Ücretsiz ilan ver </a>
                </div>
            </div>
            <div class="col-sm-3  page-sidebar-right">
                <aside>
                    <div class="panel sidebar-panel panel-contact-seller">
                        <div class="panel-heading">Bayi ile İletişime Geçin</div>
                                    ...
                                    <div class="company-logo-thumb col-md-9 row">
                                        <div class="m-l-5">
                                            ...
                                            @if($result[0]->user->mobile_verify == 1)
                                                <div class="btn btn-xs m-t-5 btn-success">Telefon Doğrulandı</div>
                                            @endif
                                    ...
                                    <div class="text-leftt clearfix">
                                        ...
                                        @if($result[0]->user->type == 'c')
                                            @if($result[0]->user->telephone)
                                                <p>Telefon: <strong>{{ ucfirst($result[0]->user->telephone) }}</strong></p>
                                            @endif
                                            @if($result[0]->user->fax)
                                                <p>Faks: <strong>{{ ucfirst($result[0]->user->fax) }}</strong></p>
                                            @endif
                                            @if($result[0]->user->address)
                                                <p>Konum: <strong>{{ ucfirst($result[0]->user->address) }}</strong></p>
                                            @endif
                                        @endif
