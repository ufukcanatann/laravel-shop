    ...
    <div class="main-container">
        <div class="container">
            ...
            <div class="col-md-9">
                ...
                <a href="javascript:;" class="pull-right m-t-10"><i class="fa fa-line-chart m-l-10" aria-hidden="true"></i> Ziyaretler <span class="">{{ floor($data->visit/2) }}</span></a>
            </div>
            <div class="col-md-3">
                <div class="pull-right backtolist">
                    <a href="javascript:;" onclick="goBack()"> <i class="fa fa-angle-double-left"></i>  Sonuçlara Dön</a>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-sm-9 page-content col-thin-right">
                    <div class="inner inner-box ads-details-wrapper">
                        <h1 class="auto-heading"><span class="auto-title left"> {{ ucfirst($data->title) }} </span>
                            <span class="auto-price pull-right"><span style="font-size: 18px">Kalan süre: </span><span id="time"></span></span>
                        </h1>
                        ...
                        <div class="row ">
                            <div class="col-sm-12 automobile-left-col">
                                <div class="a2a_kit a2a_kit_size_32 a2a_default_style pull-right m-b-5" style="margin-top: -18px;">
                                    <div class="col-sm-4"><strong>Açılış Fiyatı: </strong><span class="sonteklif-baslik"> 2.500</span> TL</div>
                                    <div class="col-sm-4"><strong>Son Teklif: </strong><span class="sonteklif-baslik"> 11.500</span> TL</div>
                                    <div class="col-sm-4">
                                        <div class="input-group">
                                            <input type="text" class="form-control">
                                            <span class="input-group-btn">
                                                <button type="button" class="btn btn-primary">Teklif Ver</button>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-5">
                                    <h5 class="list-title"><strong>Anahtar bilgi</strong></h5>
                                    <ul class="list-group">
                            ...
                            <div class="col-md-12 m-b-20">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <ul class="list-circle">
                                                @foreach($row['group_fields'] as $val)
                                                    <?php $i++; ?>
                                                    @if($i> 2 )
                                                        <h4 class="col-md-12 row " data-if="0" data-str="{{ $row['group_title'] }}" id="main_{{ $count }}" onclick="show_hide(this)"><a href="javascript:void(0)">Tümünü göster {{ $row['group_title'] }}</a></h4>
                                                        <?php break; ?>
                                                    @else
                                ...
                                <div class="content-footer text-left m-t-20">
                                    ...
                                    <button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#contactModalId"> <i class="fa fa-envelope-o" aria-hidden="true"></i> İletişim </button>
                                  @endif
                                    <a @if($data->user->mobile_verify == 1) title="Phone  Verified" @endif id="Mobile_Contact" data-toggle="modal" data-target="#showPhone" class="btn  btn-info btn-lg" onclick="$('#phone_hide').addClass('hidden'); $('#phone_show').removeClass('hidden');"><i class=" icon-phone-1"></i> <span id="phone_hide">Numarayı göster</span>
                                    ...
                                    @if($setting->live_chat == 1)
                                        ...
                                        ?>
                                            @if( $data->user->id != Auth::user()->id && $data->user->is_login == 1 && $is_locked =='' )
                                                <button  class="btn btn-default btn-lg chathead" data-for="{{ $data->user->id }}"><i class=" icon-chat"></i> Şimdi sohbet et </button>
                                            @endif
                                        @endif
                                     @endif
                ...
                <div class="col-sm-3  page-sidebar-right">
                    <aside>
                            ...
                            <div class="panel-content user-info">
                                <div class="panel-body p-0">
                                    <div class="seller-info" style="position: relative">
                                        ...
                                        <div class="company-logo-thumb col-md-6 row">
                                            <div class="m-l-5">
                                                ...
                                                @if($data->user->is_verified == 2)
                                                    <div class="btn btn-xs btn-success"> Doğrulanmış Kullanıcı </div>
                                                @endif
                                                @if($data->user->mobile_verify == 1)
                                                    <div class="btn btn-xs btn-success m-t-5">Telefon Doğrulandı</div>
                                                @endif
                                        ...
                                        <div class="text-leftt clearfix">
                                            ...
                                            @if($data->user->type == 'c')
                                                @if($data->user->telephone)
                                                    <p>Telefon: <strong>{{ ucfirst($data->user->telephone) }}</strong></p>
                                                @endif
                                                @if($data->user->fax)
                                                    <p>FAKS: <strong>{{ ucfirst($data->user->fax) }}</strong></p>
                                                @endif
                                                @if($data->user->address)
                                                    <p>Konum: <strong>{{ ucfirst($data->user->address) }}</strong></p>
                                                @endif
                                            @endif
                                        ...
                                        <a href="{{ url('user-ads', $data->user->id) }}" class="btn btn-primary btn-block m-t-5"> <i class=" fa fa-user"></i> Kullanıcının diğer ilanları </a>
                        ...
                        @endif
                        <div class="panel sidebar-panel">
                            <div class="panel-heading">Alıcılar için Güvenlik İpuçları</div>
                            <div class="panel-content">
                                <div class="panel-body text-left">
                                    <ul class="list-check">
                                        <li> Satıcıyla halka açık bir yerde buluşun</li>
                                        <li> Satın almadan önce ürünü kontrol edin</li>
                                        <li> Yalnızca ürünü aldıktan sonra ödeyin</li>
                                    </ul>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="Email-container" class="{{ (Auth::guest())? 'hidden' : '' }}">
                    ...
                    <form id="contactForm" action="">
                        ...
                        <div class="form-group">
                            <label for="">Mesaj <span class="text-danger">*</span></label>
                            <textarea required class="form-control" name="msg" id="" rows="3" placeholder="Buraya metin ekleyin .."></textarea>
                        </div>
                        <div class="form-group">
                            <button type="submit" class="btn btn-success"> <i class="icon-mail" aria-hidden="true"></i> Gönder </button>
                        </div>
                    </form>
                </div>
            </div>
            <div class="modal-footer">
                @if(Auth::guest())
                    <a href="{{ route('register') }}" class="btn btn-primary reg_btn">Kayıt ol</a>
                @endif
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h3 class="modal-title text-center">  Güvenlik içinde iş yapın {{ $setting->title }}</h3>
                </div>
                <div class="modal-body text-center">
                    <div class="p-25">
                        <p>
                            Mümkünse, anlaşmayı kapatmak için satıcıyla şahsen görüşün. Takip edilemeyen ödeme araçlarından kaçınmaya çalışın.
                        </p>
                        <h3><i class="fa fa-phone" aria-hidden="true"></i> @if($data->user->phone == '') <span class="text-danger">Müsait değil!</span>
                            @else
                                {{ $data->user->phone }}
                                @if($data->user->mobile_verify == 1)
                                    <div class="btn btn-xs btn-success ">Telefon Doğrulandı</div>
                                @endif
                            @endif
                        </h3>
                    </div>
