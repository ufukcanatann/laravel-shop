...
@section('content')
    <style>
        ...
        .my_form{
            ...
            margin-bottom: 10px;
            border-bottom: 4px solid #3ac9d6
        }
        ...
        <div class="content">
            <div class="col-xs-12">
                <div class="page-title-box">
                    <h4 class="page-title">Grup Alanları Ekle</h4>
                    <ol class="breadcrumb p-0 m-0">
                        <li> <a href="{{ url('dashboard') }}">Kontrol Paneli</li>
                        <li class="active">Grup Alanları Ekle</li>
                    </ol>
            ...
            <div class="col-xs-12">
                ...
                <div class="card-box">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <a href="{{ route('groups.create') }}" class="btn btn-primary btn-sm pull-right m-b-10" > Yeni Grup Ekle  </a>
                        </div>
                        @if(count($groups) > 0)
                            ...
                            <div class="col-xs-12 bg-white">
                                <div class="portlet">
                                    <div class="portlet-heading bg-inverse">
                                        <h3 class="portlet-title" title="Group name">@if($val['icon']!='' || $val['image']!='')  {!! ($val['icon']!='')? '<i class="'.$val['icon'].'"></i>' : '<img src="'.asset('assets/images/groups_icons/'.$val['image'].'').'" height="38px" width="38px    ">' !!}   {{ ucwords($val['title']) }} <span class="label label-pink" title="Category"> {{ $val['group_cat']['name'] }} </span> @else {{ ucwords($val['title']) }} <span class="label label-pink" title="Category"> {{ $val['group_cat']['name'] }} </span> @endif </h3>
                                        <div class="portlet-widgets">
                                            <span class="divider"></span>
                                            <a data-toggle="collapse" data-parent="#accordion1" href="#group_{{$val['id']}}" class="collapsed" aria-expanded="false"><i class="ion-minus-round"></i></a>
                                            <span class="divider"></span>
                                            <a href="{{ route('groups.edit', $val['id']) }}"><i class="fa fa-pencil"></i></a>
                                            <span class="divider"></span>
                                            <a href="javascript:void(0)" data-obj="groups"  data-id="{{$val['id']}}" onclick="deleteRow(this)" ><i class="ion-close-round"></i></a>
                                        </div>
                                        <div class="clearfix"></div>
                                    </div>
                                    <div id="group_{{$val['id']}}" class="panel-collapse collapse @if (session('id') && $val['id'] == session('id')) in @endif"  @if (session('id') && $val['id'] == session('id') ) aria-expanded="true" @else aria-expanded="false" style="height: 0px;" @endif>
                                        <div class="portlet-body">
                                            <!-- Button trigger modal -->
                                            <div class="col-md-12">
                                                <button type="button" class="btn btn-primary btn-sm pull-right m-b-10" onclick="$('#group_id').val({{$val['id']}});$('#category_id').val({{$val['category_id']}}); $('#newFieldTitleId strong').html('({{ ucwords($val['title']) }} -> {{ ucwords($val['group_cat']['name']) }} )')"  data-toggle="modal" data-target="#newFieldModal"> Yeni Alan Ekle  </button>
                                            </div>
                                            <div class="clearfix"></div>
                                            @if( count($val['group_fields']) > 0 )
                                                @foreach( array_reverse($val['group_fields'], true) as $value)
                                                    @if($value['status'] == 1)
                                                    <div class="clearfix"></div>
                                                        <div class="row my_form">
                                                            <form action="{{route('group_fields.store')}}" id="form_Group{{$value['id']}}" method="post" enctype="multipart/form-data">
                                                                {{csrf_field()}}
                                                                <input type="hidden" value="{{$value['id']}}" name="id">
                                                                <input type="hidden" value="{{$val['id']}}" name="group_id">

                                                                <div class="form-group">
                                                                    <label class="pull-right"><a href="javasript:void(0)" data-obj="group_fields" data-id="{{$value['id']}}" onclick="deleteRow(this)"><span class="label label-danger">X</span></a></label>
                                                                ...
                                                                <div class="form-group">
                                                                    <div class="radio radio-info radio-inline">
                                                                        <input type="radio" class="is_icon" id="is_icon_{{ $value['id'] }}" data-id="{{ $value['id'] }}" value="option1" name="switch" {{ ($value['icon']!='')? 'checked' : '' }}>
                                                                        <label for="is_icon_{{ $value['id'] }}">Simge Ekle</label>
                                                                    </div>
                                                                    <div class="radio radio-inline">
                                                                        <input type="radio" class="is_image" id="is_image_{{ $value['id'] }}" data-id="{{ $value['id'] }}" value="option2" name="switch" {{ ($value['image']!='')? 'checked' : '' }}>
                                                                        <label for="is_image_{{ $value['id'] }}">Görsel Ekle </label>
                                                                    </div>
                                                                </div>
                                                                <div class="form-group @if($value['image'] != '') hidden @endif  " id="icon-div_{{ $value['id'] }}">
                                                                    <label for="">Simgeler:</label><br>
                                                                    <div class="btn-group">
                                                                        <button type="button" onclick="$('#modal_hidden_id').val({{ $value['id'] }})"  class="btn btn-default" value="" data-toggle="modal" data-target="#iconModal">Simge Ekle</button>
                                                                        <button type="button" id="myicon_{{ $value['id'] }}"  class="btn btn-primary {{ ($value['icon']!='')? '' : 'hidden' }}"> {!! ($value['icon']!='')? '<i class="'.$value['icon'].'"></i>' : '' !!}</button>
                                                                        <input type="hidden" name="icon"  id="icon_val_{{ $value['id'] }}" value="{{ ($value['icon']!='')? $value['icon'] : '' }}" >
                                                                    </div>
                                                                </div>
                                                                <div class="form-group {{ ($value['image']!='')? '' : 'hidden' }}" id="img-div_{{ $value['id'] }}">
                                                                    <label class="control-label">Görsel Simgesi:</label>
                                                                    <img src="{{asset('assets/images/groups_icons/'.$value['image'].'')}}" alt="" height="25" width="25">
                                                                    <input type="file" name="image" class="filestyle"  id="filestyle_{{ $value['id'] }}" data-input="false">
                                                ...
                                                @endforeach
                                                @else
                                                <p class="text-danger"> Henüz herhangi bir <strong> {{ ucwords($val['title']) }} </strong> grup alanı eklemediniz! </p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="newFieldTitleId">Yeni <strong class="text-info"></strong> Alan Ekle</h4>
                </div>
                <div class="modal-body">
                    <form action="{{route('group_fields.store')}}" id="form_Group" method="post" enctype="multipart/form-data">
                        {{csrf_field()}}
                        ...
                        <div class="form-group">
                            <div class="radio radio-info radio-inline">
                                <input type="radio" class="is_icon" id="is_icon_0" data-id="0" value="option1" name="switch" checked>
                                <label for="is_icon_0">Simge Ekle</label>
                            </div>
                            <div class="radio radio-inline">
                                <input type="radio" class="is_image" id="is_image_0" data-id="0" value="option2" name="switch">
                                <label for="is_image_0">Görsel Ekle </label>
                            </div>
                        </div>
                        ...
                        <div class="form-group" id="icon-div_0">
                            <label for="">Simgeler:</label><br>
                            <div class="btn-group">
                                <button type="button" onclick="$('#modal_hidden_id').val(0)"  class="btn btn-default" value="" data-toggle="modal" data-target="#iconModal">Simge Ekle</button>
                        ...
                        <div class="form-group hidden" id="img-div_0">
                            <label class="control-label">Görsel Simgesi:</label>
                            <input type="file" name="image" class="filestyle"  id="filestyle_0" data-input="false">
                        </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="modelTitleId">Simge Ekle</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-6 col-sm-offset-3">
                            <div class="form-group search-box">
                                <input type="text" id="search-input" class="form-control product-search" placeholder="Burada Ara...">
                        ...
                        <section id="medical">
                            <div class="row icon-list-demo">
                                ...
                                <div class="col-sm-6 col-md-4 col-lg-3 text-danger" id="no-matching-result">
                                    Üzgünüz, verilen isim için eşleşme bulunamadı
                                </div>
                            </div>
